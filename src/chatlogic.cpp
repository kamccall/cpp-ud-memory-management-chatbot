...
ChatLogic::ChatLogic()
{
    ...
    // _chatBot = new ChatBot("../images/chatbot.png");   // KM no longer required (commented) - moved below

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    // _chatBot->SetChatLogicHandle(this);               // KM no longer required (commented)
...
ChatLogic::~ChatLogic()
{
    ...
    // std::cout << "chatlogic destructor firing...\n";  
    delete _chatBot;
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](const std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });  // KM converted to unique_ptr<GraphNode &node>

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));   // KM create unique_ptr<GraphNode> using make_unique<GraphNode>
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            edge->SetChildNode((*childNode).get());     // KM added .get to end of both
                            edge->SetParentNode((*parentNode).get());
                            // _edges.push_back(edge.get());            // KM don't need anymore
    ...
    else
    {
  	...
  	// KM TEST print out entire nodes list
    /*
  	std::cout << "NODES LIST" << std::endl;
  	for (const auto& node: _nodes)
    {
    	std::cout << "ID: " << node->GetID() << "  ANSWER: ";
      	std::vector<std::string> answers = node->GetAnswers();
    ...
    */
    ...
    // add chatbot to graph root node
  	chatbot.SetRootNode(rootNode);                  // KM changed to chatbot.SetRooNode (since chatot is now stack object)
    rootNode->MoveChatbotHere(std::move(chatbot));  // KM used move semantics to move chatbot to rootnode ownership
